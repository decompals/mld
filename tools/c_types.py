# Nabbed with care from https://github.com/matt-kempster/m2c/blob/master/m2c/c_types.py
...
def parse_constant_int(expr: "ca.Expression", typemap: TypeMap) -> int:
    if isinstance(expr, ca.Constant):
        try:
            ...
            raise Exception(f"Failed to parse {to_c(expr)} as an int literal")
...
def parse_struct_member(
    ...
    if isinstance(type, ArrayDecl):
        if type.dim is None:
            raise Exception(f"Array field {field_name} must have a size")
    ...
    if size == 0 and not allow_unsized:
        raise Exception(f"Field {field_name} cannot be void")
    ...
    for decl in struct.decls:
        ...
        if decl.bitsize is not None:
            ...
            if ssize != salign or substr is not None:
                raise Exception(f"Bitfield {field_name} is not of primitive type")
            if width > ssize * 8:
                raise Exception(f"Width of bitfield {field_name} exceeds its type")
            if is_union:
...
def parse_c(
    ...
    try:
        ...
        if len(parts) >= 2:
            ...
            posstr = ""
        raise Exception(f"Syntax error when parsing C context.\n{msg}{posstr}")
    return ast, c_parser._scope_stack[0].copy()


def build_typemap(ast) -> TypeMap:
    typemap = TypeMap()
